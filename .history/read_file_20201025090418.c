    ...
    for(int g = 0; g < rank; g++){ /*Extra for loop compared to the determinant as we have to go to visit elements not just top row*/
        for(int h = 0; h < rank; h++){
            ...
            for(int i = 0; i < rank; i++){
                for(int j = 0; j < rank; j++){
                    if(j!=h && i!=0){
                        submatrix->data[submatrix->cols*i+j] =matrix->data[matrix->cols*i+j];
                        if(n < rank -2){
                            n++;
                        else{
                            n=0;
                ...
                cofactor->data[rank*g+h] = (-1)**(g+h)determinant(submatrix) /*Minus sign depends on location in the matrix*/
            }
        }
    }
    transpose(cofactor);
