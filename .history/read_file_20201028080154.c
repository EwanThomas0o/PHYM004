...
Matrix * subtract(Matrix* mat1, Matrix* mat2){
    if(mat1->rows != matr2->rows || mat1->cols != mat2->cols){
    ...
    for(int i = 0; i < mat1->rows; i++){
        for(int j = 0; j < mat1->cols; j++){
            sub->data[cols*i+j] = mat1->data[cols*i+j] - mat2->data[cols*i+j];
        }
    }
    return sub;
...
Matrix * create_unitary(int n){
    ...
    for(int i = 0; i < n; i++){
        for(int j = 0; j < n; j++){
            if(i == j){
                unit->data[cols*i+j] = 1;
            }
            else{
                unit->data[cols*i+j] = 0;
            }
        }
    }
    return unit;
...
Matrix *find_residues(Matrix *matrix){
    ...
    test_unit = product(matrix, inverse(matrix));
    Matrix *unit = create_unitary;

    return subtract(unit, test_unit);
}
